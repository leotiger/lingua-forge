...
if $WP plugin is-active woocommerce --quiet 2>/dev/null; then
    ...
    # Mark Test Widget EN as virtual so checkout E2E tests do not require a
    # shipping zone.  WC skips the shipping step for virtual products, which
    # allows payment methods (COD) to render immediately in WC Blocks checkout.
    if [ -n "$EN_ID" ]; then
        $WP post meta update "$EN_ID" _virtual 'yes' --quiet 2>/dev/null \
            || $WP post meta add "$EN_ID" _virtual 'yes' --quiet 2>/dev/null \
            || true
        echo "    ↳ Test Widget EN marked virtual (no shipping required)."
    fi
...
'
    ...
    # ── WC checkout page translations (DE) ───────────────────────────────────
    # WooCommerce creates Cart, Checkout, and Order Received pages on first
    # activation (EN only).  WcPageBridge::translate_wc_page_id() looks up
    # the TRID of the WC-option page and returns the translation for the active
    # language — but only if a translation exists with _lf_lang + _lf_trid.
    #
    # This block creates DE stubs for the three front-end flow pages and links
    # them to the WC source pages so the E2E checkout journey can verify that
    # WcPageBridge routes DE visitors to the right pages.
    #
    # "Order Received" is a sub-path of checkout (/checkout/order-received/)
    # in the WC blocks checkout, so we only need cart + checkout for routing;
    # the order-received URL is generated by WC from the checkout slug.
    echo "  Creating DE translations of WC core pages (cart, checkout) …"
    $WP eval '
$pages_to_translate = [
    "woocommerce_cart_page_id"     => [ "de" => [ "title" => "Warenkorb",   "slug" => "warenkorb"  ] ],
    "woocommerce_checkout_page_id" => [ "de" => [ "title" => "Kasse",        "slug" => "kasse"       ] ],
];

foreach ( $pages_to_translate as $option_key => $langs ) {
    $src_id = (int) get_option( $option_key );
    if ( ! $src_id ) {
        echo "  ↳ $option_key not set — WooCommerce not yet installed? skipping.\n";
        continue;
    }
    // Ensure source page has _lf_lang=en and a TRID.
    $src_lang = get_post_meta( $src_id, "_lf_lang", true );
    if ( ! $src_lang ) {
        update_post_meta( $src_id, "_lf_lang", "en" );
    }
    $trid = get_post_meta( $src_id, "_lf_trid", true );
    if ( ! $trid ) {
        $trid = wp_generate_uuid4();
        update_post_meta( $src_id, "_lf_trid", $trid );
    }
    foreach ( $langs as $lang => $data ) {
        ...
        // Copy the source page content so the checkout/cart blocks/shortcode
        // is present on the translation — an empty page renders no checkout form.
        $src_content = get_post_field( "post_content", $src_id );

        if ( $existing ) {
            $existing_id = $existing[0];
            $current_content = get_post_field( "post_content", $existing_id );
            if ( empty( $current_content ) && ! empty( $src_content ) ) {
                wp_update_post( [ "ID" => $existing_id, "post_content" => $src_content ] );
                echo "  ↳ Updated \"" . $data["title"] . "\" ($lang, ID $existing_id) — copied source content.\n";
            } else {
                echo "  ↳ " . $data["title"] . " ($lang) already exists (ID $existing_id), skipping.\n";
            }
            continue;
        }
        $trans_id = wp_insert_post( [
        ...
        if ( is_wp_error( $trans_id ) ) {
            echo "  ERROR creating " . $data["title"] . ": " . $trans_id->get_error_message() . "\n";
            continue;
        }
        update_post_meta( $trans_id, "_lf_lang", $lang );
        update_post_meta( $trans_id, "_lf_trid", $trid );
        echo "  ↳ Created \"" . $data["title"] . "\" ($lang, slug=" . $data["slug"] . ") → ID $trans_id  trid=$trid\n";
    }
}
'

    # ── Enable COD payment for E2E checkout ──────────────────────────────────
    # COD requires no payment credentials — perfect for a local E2E environment.
    # Always merge+write (not skip-if-enabled) so enable_for_virtual stays 'yes'
    # even when WC wrote 'no' during installation.  A virtual cart has no shipping
    # step; without enable_for_virtual='yes', WC's is_available() returns false and
    # WC Blocks renders an empty payment-methods list.
    echo "  Enabling Cash on Delivery (COD) payment gateway …"
    $WP eval '
$settings = get_option( "woocommerce_cod_settings", [] );
$settings["enabled"]            = "yes";
$settings["title"]              = $settings["title"] ?? "Cash on delivery";
$settings["order_status"]       = $settings["order_status"] ?? "processing";
$settings["enable_for_virtual"] = "yes";   // allow COD when cart has only virtual items
$settings["enable_for_methods"] = $settings["enable_for_methods"] ?? "";  // all methods
update_option( "woocommerce_cod_settings", $settings );
echo "  ↳ COD enabled (enable_for_virtual=yes).\n";
'
