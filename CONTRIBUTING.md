...
| `linguaforge_sync_templates( $post_id, $check_caps = false )` | `array{success:bool,message?:string,results?:array}` | The engine behind the "TS" (Template Sync) button in the post list Lang column — Sync's no-AI-cost sibling. Reassigns the language-specific FSE template (`_wp_page_template`, via `Sync::assign_template_if_needed()`) for every **existing** sibling in `$post_id`'s TRID group. Never calls the AI translation feature and never touches `post_content`/`post_title`/`post_excerpt`; cannot create a missing sibling (that requires translated content, i.e. an AI call). `$post_id` must be the **primary/source-language** post — restricted from the start, since this is the one entry point meant to fix every sibling's template in a single pass; calling it with a secondary-language post's ID returns `success: false`. No secondary-language safeguard is needed (unlike Sync): this action never touches content, so the back-translation content-overwrite risk those guards exist for does not apply. `$check_caps` defaults to **false**, same convention as `linguaforge_sync_translations()`. (Since 2.6.1) |
