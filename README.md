...
> **Version 2.6.7 — stable, open for testing.**
...
> Multilingual support is a fundamental need for any publishing platform. Gutenberg's name honours Johannes Gutenberg not merely for inventing the printing press, but for making the dissemination of knowledge possible at scale — a mission that stops at a language boundary if the platform does not natively cross it. We believe first-class multilingual support belongs in WordPress Core and in Gutenberg itself, not perpetually deferred to a third-party plugin ecosystem.
>
> Lingua Forge is not listed in the WordPress.org Plugin Directory — not because of code quality, security, trademark, or licensing concerns, but because WordPress.org maintains an undisclosed and ever-growing list of plugin names it considers reserved or off-limits, with no public registry, no appeal process, and no stated criteria. When gatekeeping operates without transparency, it does not protect a commons; it creates one.
> 
> Closing a door on a legitimate, well-intentioned contribution is itself a door that opens toward something less accountable. Let's not name what that less accountable future stands for. What is clear is that forces are at work within WordPress.org that do not want free multilingual support delivered for WordPress. That in itself is bad for the community, which is largely unaware of what is happening, and it compromises the platform's future — not only because of technological progress outside the ecosystem, but because of internal power structures that have contaminated the WordPress ecosystem itself.
>
> It's closing up. It's losing its spirit.
> There is an opt-in bureaucracy at work, without effective oversight. Ancient Rome suffered from this; why should WordPress have a different destiny? Decline is often imperceptible to insiders whose beliefs have taken on a religious character. In projects like WordPress, "True Believers" (search for the term and you will find Eric Hoffer) make up the army. Feel free to star not the plugin — we want to see multilingual support in Core — but the reasoning behind it. If you want to do it for the plugin as well, you're welcome to. We have even offered our domain, https://lingua-forge.com, as a gift to the WordPress community, along with this plugin, in the hope of helping the platform do a better job for all of us.
>
...
2. **Handles the full multilingual SEO surface natively — no companion plugin required** — hreflang, a self-referencing canonical aligned with it, Open Graph with `og:locale`/`og:locale:alternate`, Schema.org JSON-LD with `inLanguage` (including a language-aware BreadcrumbList), a dedicated chunked XML sitemap with hreflang alternates, native IndexNow submission of every language version, per-language `noindex`, WooCommerce product schema, Social Icons block share: URL rewriting, and AI-powered SEO content analysis in both the Settings page and the block editor sidebar. A Compatibility tab shows live detection of any installed SEO plugin and explains exactly what Lingua Forge does for each feature area.
...
The short version: Lingua Forge covers the full multilingual workflow that the paid tiers of those plugins provide — language routing, complete multilingual SEO (hreflang, Open Graph, Schema.org, sitemap, IndexNow), FSE templates, translation groups, browser language redirect, translated slugs, and a built-in WooCommerce integration — and adds a deeper AI editorial layer than any of them ships natively, at zero licensing cost. The key difference is economic: no license fees, no annual renewals, no per-word translation credits. Use the AI features and you pay your provider directly at API rates; translate manually and the cost is zero.
...
Where Lingua Forge stands apart is the combination — every item below ships in the single free plugin, and no competitor offers the whole set:

- **A complete native multilingual SEO layer** — hreflang, a self-referencing canonical, Open Graph with `og:locale`, Schema.org JSON-LD with `inLanguage` (including a language-aware BreadcrumbList), a dedicated chunked XML sitemap with hreflang alternates, native IndexNow submission of every language version, per-language `noindex`, WooCommerce product schema and OG, Social Icons block share-URL rewriting, and AI-powered SEO content analysis — with no companion SEO plugin. The other post-based plugins delegate SEO to Yoast/Rank Math; none emit `inLanguage`, and none ship IndexNow natively.
- **FSE-native from the ground up** — language-specific template *entities* (`page-de`, `header-ca`) and a one-screen scaffold → AI-translate → fix-links → fix-nav-refs workflow, plus the Language Switcher block. Polylang Pro translates template-part content but not separate template entities; WPML's Site-Editor support is a retrofit with documented open issues; MultilingualPress gets FSE only by running each language as its own WordPress site.
- **Every public Custom Post Type — included by default, excludable per type** — the Lang column, AI metabox, FSE template routing, and link fixer apply to all public CPTs with zero setup; the System tab lists every post type and lets you exclude any one from routing in a single click (system types such as `shop_order` are excluded out of the box). This is the inverse of WPML and Polylang, where each CPT stays untranslated until you enable it per type — Lingua Forge works everywhere by default and steps aside only where you tell it to.
- **Built-in WooCommerce at zero cost** — shared-stock delegation (price, SKU, stock, images, variations with translatable descriptions, categories, attribute term names in block and classic themes, product brands) plus the commerce lifecycle: order-language capture, locale-switched transactional emails, cross-language coupons, consolidated sales and reviews, HPOS-compatible. WCML and Polylang-for-WooCommerce are paid add-ons; here it is part of the free plugin.
- **WP-CLI for content translation and bulk automation** — translate, retranslate, fill missing translations, and clear caches across a whole post type from a shell or cron job. Polylang's CLI is configuration-only; WPML's is import/export-only.
- **An iterative AI editorial toolset inside the block editor** — full-page and block-level translation, content generation with multi-turn refinement, meta-description and excerpt generation, quick-translate, behavior presets, glossary, and translation memory. No competitor ships this natively, and AI cost goes straight to your provider at published API rates — no credit intermediary.
...
The handful of Language Router filters most relevant here:
`lf_primary_language`, `lf_languages_list`, `lf_lang_force_locale`,
`lf_lang_fallback_map`, `lf_lang_default_fallback`, `lf_base_domain`,
`lf_hreflang_mode`, `lf_hreflang_x_default`, `lf_i18n_overrides_dir`, and
`linguaforge_translation_languages` (AI target-language list). Full
signatures, defaults, and every other hook in the plugin:
**[HOOKS.md](HOOKS.md)**.
...
- **Third-party integration API** — 69 hooks across Router, AI, SEO, and WooCommerce sub-modules; full reference in [HOOKS.md](HOOKS.md). Two public REST endpoints. Public PHP function `linguaforge_trigger_translation()`. Integration walkthrough in `CONTRIBUTING.md`.
