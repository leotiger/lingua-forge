...
- **Ported Agnosis's 2026-07-24 fixes to `dev/bin/translate-missing.php`** (the `composer translate-missing` i18n helper), keeping this plugin's own architecture (`lf_`-prefixed functions, `lf_build_request()`/`lf_extract_response()` dispatch) and domain specifics (`DOMAIN_PROMPT`, `LOCALE_NAMES`, `lingua-forge-*.po` naming) untouched:
  ...
  - **Same-day follow-up fix, ported after landing in Agnosis:** `lf_parse_entries()`'s plural-entry scan previously computed one `multiline_warning` flag for the *whole* entry, and its inner `while()` loop stopped the instant it hit a line that didn't start with `msgstr[` — which is exactly what a wrapped slot's own continuation line looks like. So a single wrapped `msgstr[0]` didn't just get (correctly) skipped — it silently truncated discovery of every slot *after* it in that entry, meaning higher-index slots were never even considered for translation. Now each slot's continuation lines are consumed and concatenated before scanning continues to the next `msgstr[N]`, `multiline_warning` is tracked per-slot, and the work-list loop's skip check moved from gating the whole entry to gating only that one slot. A `--dry-run` re-run against this plugin's own `.po` files went from 46 detected items across 15 locales to **52 across 17** — the 6 newly-discovered items were exactly this failure mode.
...
_Landed after the 2.6.7 zip was already built and deployed — dev-only, `dev/` never ships in the plugin release, so this did not change what 2.6.7 users received. Logged here rather than under a new version bump since it has no plugin-functional effect._

- **New `composer translate-missing` command** (`dev/bin/translate-missing.php`) AI-fills every empty `msgstr`/`msgstr[N]` across all 26 bundled locale `.po` files in one pass, replacing the fully-manual per-string translation step of the i18n pipeline (previously: open each `.po` by hand). Supports all three providers the plugin itself supports — Anthropic (default), OpenAI, Gemini — via `--provider=`; if that flag is omitted at an interactive terminal, the script asks which provider to use first (Enter for Anthropic) before anything else, then prompts for that provider's API key too if the matching env var (`ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY`) isn't set — both prompts hide input and are used for that run only, never persisted. Writes translations directly into the `.po` files, saving after every batch (not just once a whole locale finishes) so an interrupted run keeps whatever was already translated; `--time-budget=SECONDS` lets the script stop itself cleanly before a new batch starts, for use under a wrapper/tool with its own process timeout. Mirrors — and was reconciled feature-for-feature with — the sibling Agnosis project's own `composer translate-missing` tool, adapted to this plugin's locale list, file naming, and domain vocabulary. Not covered by `composer lint`/`qa` (`dev/bin/` isn't in `phpcs.xml.dist`'s scan list). Flags: `--dry-run`, `--locale=xx`, `--limit=N`, `--batch-size=N`, `--provider=NAME`, `--model=NAME`, `--time-budget=SECONDS`. (`dev/bin/translate-missing.php` NEW, `dev/composer.json`, `CONTRIBUTING.md`, `dev/README.md`)
