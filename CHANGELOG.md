...
### Fixed
- **WP site locale injected into valid content-language list** — `Context::languages()` unconditionally added `get_locale()` to the routing language list. When the WordPress site language (e.g. `en_US`) is not a content language, its two-char code (`en`) entered the routing system, the language switcher rendered a spurious EN option, and clicking it wrote an `lf_lang=en` cookie that caused `CatalogQuery` to return zero products on every subsequent non-prefixed page. The locale is now skipped when `linguaforge_primary_language` is explicitly set — at that point the WP site locale is the admin UI language, not a content language. (`language-router/includes/class-context.php`)
