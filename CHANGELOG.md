...
- **Hardcoded `'ca'` fallback in `Context::source_language()`** — `get_option( 'linguaforge_primary_language', 'ca' )` and the `?: 'ca'` guard were both hardcoded to Catalan. On a fresh install, any request that fired before the admin completed first-time setup (automated health checks, search-engine crawls) was silently routed as if Catalan were the source language. Both defaults changed to `''`; callers that require a non-empty language check the result explicitly. (`class-context.php`)
