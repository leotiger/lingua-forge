...
- **Subdomain routing mode** — language subdomains (`de.example.com`, `fr.example.com`) are now supported as an alternative to path prefixes (`example.com/de/`). Select the URL structure in **Settings → Lingua Forge → Router → URL structure**. The source language always serves from the root domain; all other languages use either a path prefix or a subdomain depending on the selected mode. Switching modes requires a permalink flush and a cache clear.

  Implementation spans six classes:
  - `Context` — new `routing_mode()`, `base_domain()`, and `lang_base_url()` helpers; `detect_lang_from_host()` step added at position 0 in both language detection paths; `set_lang_cookie()` scopes the `lf_lang` cookie to the apex domain (`.example.com`) in subdomain mode so it is shared across all language subdomains.
  - `Manager` — skips path-prefix rewrite rules in subdomain mode; `lang_permalink()` generates subdomain URLs (`https://de.example.com/page/`) instead of path-prefix URLs.
  - `Redirector` — `allow_lang_subdomains()` whitelists language subdomains for `wp_safe_redirect()` cross-domain redirects; search redirect adapted for subdomain mode; `redirect_search_under_lang_prefix()` returns early in subdomain mode (no path prefix to detect).
  - `Hreflang` — paged and archive/home branches now use `lang_base_url()` so generated `<link rel="alternate">` `href` values are correct in both modes.
  - `Switcher` — search URL and non-singular URL construction both subdomain-aware.
  - `LinkFixer` — `$target_prefix` used for internal-link scanning derives from `lang_base_url()` in subdomain mode.

  **Server prerequisite:** wildcard DNS (`*.example.com → server IP`) or per-subdomain DNS records, plus a wildcard TLS certificate covering all language subdomains, must be configured before switching to subdomain mode. WordPress itself needs no special configuration beyond the setting.

  The `lf_base_domain` filter is available to override the auto-derived base domain when `home_url()` includes a `www` prefix or a non-apex hostname.

### Fixed

- **Translated pages auto-added to navigation menus** — WordPress's "automatically add new top-level pages" menu option is language-unaware: publishing a French or German page would insert it into every auto-add menu, including the primary source-language menu. A `publish_page` hook at priority 11 (after WordPress's own handler at priority 10) now removes any just-added menu item whose page carries a non-source-language `_lf_lang` value. Source-language pages are unaffected; `translate_menu_items()` continues to handle dynamic URL swapping at render time.

- **Language switcher invisible on archive, category, tag, and author pages** — `get_languages()` called `get_the_ID()` and returned an empty array whenever no post was in context (archives, blog index without a static page, search results). In subdomain routing mode this meant the switcher was missing on every non-singular URL, which is a hard blocker for multi-language navigation. Fix: when `get_the_ID()` returns nothing, a URL-rewrite-only map is built from all configured languages (`array_fill_keys`) and each entry is passed to `translate_current_url()` with a null post ID, falling through to the existing non-singular/subdomain URL rewrite logic. Singular pages with a full translation group are unaffected.

- **Translate Navigation generates wrong URLs in subdomain routing mode** — the "Translate Navigation" button in Settings → Router rewrote internal page URLs using path-prefix logic (`/de/contact/`) regardless of routing mode. In subdomain mode the URLs are now correctly rewritten as subdomain-based (`de.example.com/contact/`), using `lang_base_url()` for the host swap while preserving the path as-is.

- **Source-language pages redirected to wrong language when a cross-language cookie is present** — in path mode, visiting a source-language URL (no `/lang/` prefix) after browsing a translated page left a stale `lf_lang` cookie (e.g. `fr`) which `detect_lang()` / `detect_lang_safe()` honoured, causing `LF_LANG` to be set to `fr` instead of the source language. `handle_singular_redirect()` then found the French translation and redirected away from the page the visitor explicitly navigated to. Fix: in path mode, a non-empty URL path segment that does not match any known language code is treated as an authoritative source-language signal — both detection methods return `source_language()` immediately, bypassing cookie and browser-header steps. The homepage `/` is unaffected (empty first segment → cookie/browser detection still runs for first-visit language preference).

- **Fix Navigation References — source language and wrong base name** — two bugs in `ajax_fix_fse_nav_refs()`: (1) source-language template parts were incorrectly rejected as invalid targets, leaving the source-language header/footer with unfixed navigation references after a Fix Nav Refs run; (2) when a navigation object already held a wrong-language reference (e.g. `navigation-it` assigned to the DE template), the base name derivation stripped the wrong suffix and produced a double-suffixed slug (`navigation-it-de`) instead of the correct `navigation-de`. Fix: source-language validation guard removed; base name is now derived by reading `_lf_lang` meta from the currently referenced navigation post and stripping that lang suffix before appending the target language.

- **Language switcher — SVG globe icon collapsed to zero size** — the switcher's `icon` and `icon-label` display modes use an inline SVG globe icon. The old CSS sized it via `--wp--custom--fixed-bottom-bar--icon-size`, a theme-specific variable that is undefined on most sites, causing `calc()` to produce an invalid value and the icon to render at 0×0 pixels. The CSS rule is replaced with a generic `.lsflr-toggle .lsflr-current svg { width: 1.2em; height: 1.2em; fill: currentColor; }` that is independent of any theme variable.

- **Language switcher — dropdown overflows viewport on right-aligned placements** — the submenu panel could extend beyond the right edge of the browser window when the switcher was placed near the right side of a header or footer. A small inline `<script>` (IIFE, `getBoundingClientRect`) measures the submenu's actual rendered position on load and on every `resize` event and adds the `.lsflr-auto-right` class to the switcher root when overflow is detected, flipping the panel to open right-to-left. No new external asset; the script is inlined per-instance and scoped to each switcher's unique ID.

- **Language switcher — white background and invisible text on dark-themed sites** — the switcher panel used a hardcoded `background: #fff` fallback that made the dropdown unreadable on sites with a dark background. The CSS variables are now seeded from the FSE global-style tokens (`--wp--preset--color--base` for background, `--wp--preset--color--contrast` for text) with CSS system-color fallbacks (`Canvas` / `CanvasText`) that automatically track the OS light/dark preference via `color-scheme: light dark`. Sites using custom FSE palette names can override `--lsflr-bg` and `--lsflr-color` on any ancestor element without touching plugin CSS.

### Maintenance

- **`build-zip.sh` — permission normalisation** — the rsync step preserved source file permissions, which could vary across developer machines and produce inconsistent ZIP archives (e.g. executable bits on PHP files). A `find … -exec chmod` pass now sets 0755 on directories and 0644 on files after rsync and before the ZIP is created.
