...
- **Site Editor navigation language filtering (canvas + sidebar)** — the Site Editor now shows the correct language's pages in the navigation canvas and the Inserter sidebar. `QueryFilter::register_rest_nav_lang_meta()` exposes `_lf_lang` as a REST-readable field on `wp_navigation` posts and registers `lf_lang` as a recognised collection parameter on the `/wp/v2/pages` endpoint so the REST layer can filter by language. `Scripts::enqueue_admin_lang_scripts()` resolves the active language synchronously in PHP from `$_GET` parameters (canvas URL) and injects it as an inline `lfNavLang` data object. `nav-lang-filter.js` registers a `wp.apiFetch` middleware that appends `?lf_lang=<code>` to `/wp/v2/pages` requests and patches `history.pushState` / `replaceState` with a `maybeInitAsyncIfChanged()` guard (keyed on nav post ID) so the sidebar re-fetches on actual navigation changes but ignores internal Site Editor SPA transitions (panel opens, block selections, canvas reloads). `add_post_type_support('wp_navigation', 'custom-fields')` is gated on `current_user_can('edit_posts')` to avoid side-effects on non-admin requests.
...
- **`extractLangFromSlug` regex failing for multi-char language codes** — `nav-lang-filter.js` SPA-navigation path used `/-([a-z]{2})$/`, silently producing an empty string for slugs like `primary-navigation-zh-tw`. Regex updated to `/-([a-z]{2,}(?:-[a-z]{2,})?)$/`; the PHP REST param pattern aligned to `^[a-z]{2,}(-[a-z]{2,})?$`.
